...
Starting points: `assets/azure-pipelines.yml` and `assets/github-actions.yml` — adapt and test them in the
customer's own repository before pointing anything at production.

The pipeline is **rewritten, not patched**. The build stage survives almost unchanged; the release stage is
deleted and replaced by five CLI commands.
...
| `dotnet publish '*.sln' -c Release -o builds/publish` (solution-wide, Windows) | `dotnet publish <web>.csproj -c Release -o builds/publish -f net8.0 -a x64 --os linux` — **one project**, the startup web project, targeted at Linux x64. Match `-f` to the solution's target framework |
| `Litium.Cloud.Tools.Targets` not present | Added to the web project; it writes `litiumcloud.manifest.json` (entry point, .NET version, probe paths) at publish. See `references/code-changes.md` |
...
| — | New: `dotnet tool update -g litium.cloud.cli --no-cache` |
| — | New: download the service principal certificate (secure file / repository secret) |
| — | New: `litium-cloud auth login --service-principal --username <service-principal-id> --certificate <file>` |
| — | New: `litium-cloud artifact create --subscription <id> --artifact-type dotnet --file-path <publish-folder> --no-progress -o json`, read `data.artifactId` |
| — | New: poll `litium-cloud artifact show --artifact <id> -o json` until `status` is `Ready` |
| PowerShell: msdeploy `recycleApp` StopAppPool | Dropped. Production deployments are rolling; no downtime step exists |
| PowerShell: msdeploy `sync` contentPath with `AppOffline` | `litium-cloud app deploy --subscription <id> --environment <id> --app <id> --artifact <artifact-id> -o json`, read `jobId` |
| PowerShell: msdeploy `recycleApp` StartAppPool | Dropped. Poll `litium-cloud status show --job <job-id> -o json` instead |
| Deployment "succeeded" when msdeploy exited 0 | The job is finished when `completedAt` is set, and succeeded when no entry in `items` has `failed` set to `true`. On failure print `litium-cloud status logs --job <job-id>` and fail the pipeline |

**Caveat worth writing into the pipeline:** a parent job reports *completed* as soon as it finishes, whether or
not the work succeeded, and a child item can be *failed* underneath it. Checking `completedAt` alone marks a
broken deploy as green — check the `items` array for `failed` as well, as both assets do.

Artifact statuses are `Initiated`, `Uploading`, `Processing`, `Ready` and `Failed`. `dotnet` and `nextjs`
artifacts are built after upload, so several minutes in `Processing` is normal; raise the retry count rather
than the sleep interval. On `Failed`, the artifact id doubles as the id of its build job, so
`litium-cloud status logs --job <artifact-id>` prints the build log.

## Variable mapping

| Legacy variable | Becomes | Secret |
|---|---|---|
| `PublishName` (msdeploy site) | `app` — the installed app id, for example `litium` | No |
| `PublishServer` (`https://<server>:8172/msdeploy.axd`) | `subscription` + `environment` ids | No |
| `PublishUsername` | The service principal id (`service.<name>@cloud`) | Yes |
| `PublishPassword` | The service principal **certificate**, stored as an Azure DevOps secure file or a GitHub Actions secret | Yes |
| `Source`, `ExtractedSource` | `publishPath` — the folder `dotnet publish` wrote | No |
...
| Litium NuGet user/password | Kept, still needed for restore and for installing the CLI | Yes |
| — | New: `artifactType` (`dotnet`, or `nextjs` for the storefront) | No |
...
pipelines — the production principal should never be able to touch anything else.
...
under **Pipelines > Library > Secure files**, and a variable group with the Litium NuGet credentials and the
service principal id (secret), plus `subscription`, `environment` and `app` (not secret). `DownloadSecureFile@1`
puts the certificate on the agent and `$(<name>.secureFilePath)` is its absolute path — pass that, not a
relative path. Azure DevOps sets `TF_BUILD`, so the CLI runs non-interactively: no browser, no confirmation
prompts, `--no-progress` on by default.
...
line breaks intact). The workflow writes the certificate to `$RUNNER_TEMP/cert.pem` with `chmod 600`, passes ids
between steps through `$GITHUB_OUTPUT`, and parses JSON with `jq`, which is preinstalled on GitHub-hosted
runners. GitHub Actions sets `CI=true`, which puts the CLI in the same non-interactive mode.
...
Same five commands, three changes: `artifactType` is `nextjs`, the app id is the storefront app's, and the
publish step becomes a Node build (`npm ci`, `npm run build`) followed by copying the files to upload into the
publish folder — or no copy at all, with a `.litiumcloudignore` in the repository excluding `node_modules`,
`.git` and build output. The .NET SDK, the NuGet feed and the tool install stay, because the CLI is a .NET tool.

## Pipeline behaviour to keep in mind

- **Add `-o json` to every command you parse.** Never parse the table output.
...
- **Do not print secrets.** Echo the command line without the values, as both assets do.
