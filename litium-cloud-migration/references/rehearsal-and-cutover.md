...
| **Apply the Litium platform manifest** (show the command, confirm target with `context show`, get a yes); follow the job with `status logs --follow` | partner | after commit | measured (longest step after the upload) | `status show` reads `Status: completed` with no *failed* item under **Details**; `app show` reads `State: Running`; back office opens on the system domain with `/Litium` | fix and re-apply, or `app delete` and reinstall |
