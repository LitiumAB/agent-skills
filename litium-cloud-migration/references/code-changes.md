...
The Litium platform app loads **only `appsettings.json` and `appsettings.production.json`**, in every environment. `appsettings.Staging.json`, `appsettings.Test.json`, Magic Chunk or XDT transforms and pipeline variable substitution have no effect. The app-configuration page also says that `ASPNETCORE_ENVIRONMENT` can load an additional `appsettings.<name>.json`; the two statements conflict, so treat any file other than those two as not loaded, and confirm on https://docs.litium.dev/cloud/serverless/guides/configure/app-configuration-and-secrets.md if you must rely on one. **Never set `ASPNETCORE_ENVIRONMENT=Development`**; the app does not start.
...
| Logging | Auto-patching is always on: at startup the platform adjusts the NLog configuration so that file targets are disabled (they become a no-op target), auto-reload is off, and log output reaches Litium Insights. Nothing is written to `litium.log` | Keep `nlog.config` in the artifact; keep logging through the normal logger. Remove code that reads, tails, rotates or emails log files, and any custom log-file path setting. A separate private .NET web app wires up Insights itself with the `Litium.Cloud.NLog.Extensions` package (docs: .NET web app) |
| Distributed locking | The platform patches the deployed Litium assemblies for cloud-compatible distributed locking | Nothing to change; do not pin or replace the lock packages yourself |
